...
internal class DeskChangeStatusCommandHandler(IDeskRepository _deskRepository, BookingSystemDbContext _dbContext) : IRequestHandler<DeskChangeStatusCommand, Unit>
{
    public async Task<Unit> Handle(DeskChangeStatusCommand command, CancellationToken cancellationToken)
    {
        ...
        if (desk is null)
            throw new DomainException($"Desk with provided id: {command.Id} not found", (int)DeskErrorCode.NotFound);
